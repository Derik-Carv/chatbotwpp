...
async function humanControl(message, client) {
    console.log('verificando se usuário já está listado como ignorado...')
    const user = message.from;
    console.log(ignoreNow.listUsers().includes(user), 'list')
    console.log(user, 'user')
    if (ignoreNow.listUsers().includes(user) === false) {
        console.log('Começando a ignorar usuário.');
        // Adiciona o usuário à lista de ignorados
        ignoreNow.addUser(user);
        human = true;

        // Exibe a lista de usuários ignorados no console
        console.log("Usuários ignorados atualmente:", ignoreNow.listUsers());

            if (ignoreActionOn === true) {
                setTimeout(() => {
                    ignoreNow.removeIgnore(user);
                    console.log('Usuário removido da lista de ignorados.');
        ...
        console.log(`usuário encontrado em lista de ignorados...`);
